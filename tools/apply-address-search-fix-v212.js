...
const backupDir = fs.mkdtempSync(
  path.join(os.tmpdir(), 'mt-optics-address-fix-v212-')
);
...
backendSource = backendSource.replace(
  cspPattern,
  `// MT_OPTICS_ADDRESS_CSP_FIX_V212\n` +
...
if (!indexSource.includes('MT_OPTICS_ADDRESS_SCRIPT_FIX_V212')) {
  indexSource = indexSource.replace(
    newScriptUrl,
    `${newScriptUrl}" data-fix="MT_OPTICS_ADDRESS_SCRIPT_FIX_V212`
  );
}
...
const replacement = String.raw`  // MT_OPTICS_ADDRESS_EMBED_FIX_V212
  function bindAddressSearch(scope) {
    scope.querySelectorAll('[data-mtf-address-search]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        document.getElementById('mtf-postcode-layer-v212')?.remove();

        const overlay = document.createElement('div');
        overlay.id = 'mtf-postcode-layer-v212';
...
console.log('[완료] MT옵틱스 주소검색 차단 해결 패치 v2.1.2 적용 완료');
