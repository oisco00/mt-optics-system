...
set BACKUP_DIR=_backup\mt-optics-v316-%date:~0,4%%date:~5,2%%date:~8,2%_%time:~0,2%%time:~3,2%%time:~6,2%
...
for %%F in (frontend\final-enhancements-v*.js) do if /I not "%%~nxF"=="final-enhancements-v316.js" del /Q "%%F" >nul 2>nul
for %%F in (frontend\postcode-kakao-v*.js) do if /I not "%%~nxF"=="postcode-kakao-v316.js" del /Q "%%F" >nul 2>nul
for %%F in (APPLY_FINAL_ENHANCEMENTS_V*.cmd) do if /I not "%%~nxF"=="APPLY_FINAL_ENHANCEMENTS_V316.cmd" del /Q "%%F" >nul 2>nul
...
node --check frontend\final-enhancements-v316.js || goto fail
node --check frontend\postcode-kakao-v316.js || goto fail
