...
async function upsertCustomer(conn, payload, userId) {
  ...
  const updateData = {
    ...
    phone: normalizePhoneValue(payload.phone) || undefined,
    mobile: normalizePhoneValue(payload.mobile) || undefined,
  ...
  if (rows.length === 0) {
    ...
    const values = [code, name, clean(payload.business_no), normalizePhoneValue(payload.phone) || null, normalizePhoneValue(payload.mobile) || null, clean(payload.address) || clean(payload.road_address) || clean(payload.jibun_address), clean(payload.postal_code), clean(payload.road_address), clean(payload.jibun_address), clean(payload.detail_address), clean(payload.address_type), clean(payload.region), toNumber(payload.opening_receivable), updateData.memo || null, userId || null, userId || null];
...
async function upsertCustomerSite(conn, payload, userId) {
  ...
  const common = {
    ...
    phone: normalizePhoneValue(payload.phone) || null,
    mobile: normalizePhoneValue(payload.mobile) || null,
