...
function createFinalEnhancementsRouter() {
  ...
  // V316 stable report APIs. These endpoints avoid ONLY_FULL_GROUP_BY errors and keep report layouts independent.
  router.get('/reports-v316/customer-sales', requirePermission('orders.read'), asyncRoute(async (req, res) => {
    const pool = await getPool();
    await ensureSalesManagersRuntime(pool);
    const dateFrom = clean(req.query.date_from) || oneMonthAgoKst();
    const dateTo = clean(req.query.date_to) || todayKst();
  ...
  router.get('/reports-v316/customer-payments', requirePermission('payments.read'), asyncRoute(async (req, res) => {
    const pool = await getPool();
    await ensureSalesManagersRuntime(pool);
    const dateFrom = clean(req.query.date_from) || oneMonthAgoKst();
    const dateTo = clean(req.query.date_to) || todayKst();
  ...
  router.get('/reports-v316/monthly-sales-matrix', requirePermission('orders.read'), asyncRoute(async (req, res) => {
    const pool = await getPool();
    await ensureSalesManagersRuntime(pool);
