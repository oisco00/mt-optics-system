...
set BACKUP_DIR=_backup\mt-optics-v317-%date:~0,4%%date:~5,2%%date:~8,2%_%time:~0,2%%time:~3,2%%time:~6,2%
...
for %%F in (frontend\final-enhancements-v*.js) do if /I not "%%~nxF"=="final-enhancements-v317.js" del /Q "%%F" >nul 2>nul
for %%F in (frontend\postcode-kakao-v*.js) do if /I not "%%~nxF"=="postcode-kakao-v317.js" del /Q "%%F" >nul 2>nul
for %%F in (APPLY_FINAL_ENHANCEMENTS_V*.cmd) do if /I not "%%~nxF"=="APPLY_FINAL_ENHANCEMENTS_V317.cmd" del /Q "%%F" >nul 2>nul
...
node --check frontend\postcode-kakao-v317.js || goto fail
