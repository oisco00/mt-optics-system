(() => {
  ...
  function toast(message, type = 'info') {
    let el = document.getElementById('mt-v316-toast');
    if (!el) {
  ...
  function injectStyle() {
    if (document.getElementById('mt-v316-style')) return;
    const style = document.createElement('style');
    style.id = 'mt-v316-style';
    style.textContent = `
      .mt-v316-num, input.mt-v316-num{text-align:right!important;font-variant-numeric:tabular-nums}.mt-v316-hidden{display:none!important}
      .mt-v316-report-page{padding:28px}.mt-v316-report-head{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:16px}.mt-v316-report-head h1{margin:0;font-size:28px}.mt-v316-tabs{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0}.mt-v316-tabs button{border:0;border-radius:999px;padding:10px 16px;font-weight:900;cursor:pointer;background:#e2e8f0}.mt-v316-tabs button.active{background:#2563eb;color:#fff}.mt-v316-card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:18px;box-shadow:0 8px 24px rgba(15,23,42,.04);margin-bottom:14px}.mt-v316-filters{display:grid;grid-template-columns:repeat(4,minmax(160px,1fr)) auto auto auto;gap:12px;align-items:end}.mt-v316-filters label{display:block;font-weight:800;margin-bottom:6px}.mt-v316-filters input,.mt-v316-filters select{width:100%;height:42px;border:1px solid #cbd5e1;border-radius:10px;padding:0 11px}.mt-v316-btn{border:0;border-radius:10px;padding:12px 16px;font-weight:900;cursor:pointer;background:#e2e8f0}.mt-v316-btn.primary{background:#2563eb;color:white}.mt-v316-btn.green{background:#059669;color:white}.mt-v316-paper{background:#fff;border:1px solid #cbd5e1;margin-top:14px;padding:18px;min-height:220px}.mt-v316-paper h2{text-align:center;margin:0 0 5px}.mt-v316-paper .period{text-align:center;color:#475569;margin-bottom:14px}.mt-v316-table{width:100%;border-collapse:collapse;font-size:13px}.mt-v316-table th,.mt-v316-table td{border:1px solid #94a3b8;padding:6px 8px}.mt-v316-table th{background:#f1f5f9;text-align:center;cursor:grab;user-select:none}.mt-v316-table .num{text-align:right}.mt-v316-table tr.subtotal{background:#fff7ed;font-weight:900}.mt-v316-table tr.grand{background:#e0f2fe;font-weight:900}.mt-v316-empty{text-align:center;color:#64748b;padding:38px}.mt-v316-muted{font-size:12px;color:#64748b;margin-top:8px}
      @media print{.nav,.sidebar,header,.topbar,.mt-v316-no-print,.mt-v316-report-head,.mt-v316-tabs,.mt-v316-filters{display:none!important}.mt-v316-report-page{padding:0}.mt-v316-card{box-shadow:none;border:0}.mt-v316-paper{border:0;margin:0;padding:0}.mt-v316-table th,.mt-v316-table td{font-size:11px;padding:4px}}
    `;
  ...
  function formatNumberInput(input) {
    if (!input || input.dataset.v316NumberBound || !numericInputCandidate(input)) return;
    input.dataset.v316NumberBound = '1';
    input.classList.add('mt-v316-num');
    ...
    if (form && !form.dataset.v316NumberSubmitBound) {
      form.dataset.v316NumberSubmitBound = '1';
  ...
  async function ensureManagerField(form) {
    if (!form || form.querySelector('[data-v316-sales-manager]')) return;
    ...
    div.dataset.v316SalesManager = '1';
  ...
  function hideUnnecessarySiteButtons(root = document) {
    root.querySelectorAll('button').forEach(btn => {
      if ((btn.textContent || '').trim() === '납품처') btn.classList.add('mt-v316-hidden');
    });
  }

  function addReportsMenu() {
    const nav = document.querySelector('.sidebar nav, .sidebar, aside, #app nav');
    if (!nav || nav.querySelector('[data-v316-report-menu]')) return;
    const btn = document.createElement('button');
    btn.type='button'; btn.dataset.v316ReportMenu='1'; btn.className='nav-item'; btn.textContent='출력보고서'; btn.style.cssText='width:100%;text-align:left;margin-top:4px;';
  ...
  const REPORTS = {
    sales: { title:'기간별 거래처별 판매현황', api:'/final/reports-v316/customer-sales' },
    payments: { title:'기간별 거래처별 수금현황', api:'/final/reports-v316/customer-payments' },
    monthly: { title:'월별 판매현황', api:'/final/reports-v316/monthly-sales-matrix' }
  };
  ...
  async function renderReports(type = currentReportType) {
    ...
    if (!app) return;
    const list = await salesManagers();
    app.innerHTML = `<main class="mt-v316-report-page"><div class="mt-v316-report-head"><div><h1>출력보고서</h1><p>보고서별 화면 조회 후 인쇄 또는 엑셀 다운로드합니다. 표 머리글은 드래그하여 컬럼 순서를 바꿀 수 있습니다.</p></div><button type="button" class="mt-v316-btn" id="v316-manager-admin">영업담당자 관리</button></div><div class="mt-v316-tabs mt-v316-no-print"><button data-report="sales" class="${type==='sales'?'active':''}">기간별 거래처별 판매현황</button><button data-report="payments" class="${type==='payments'?'active':''}">기간별 거래처별 수금현황</button><button data-report="monthly" class="${type==='monthly'?'active':''}">월별 판매현황</button></div><section class="mt-v316-card mt-v316-no-print"><div class="mt-v316-filters"><div><label>시작일</label><input id="v316-from" type="date" value="${monthStart()}"></div><div><label>종료일</label><input id="v316-to" type="date" value="${today()}"></div><div><label>영업담당자</label><select id="v316-manager"><option value="">전체</option>${list.map(m=>`<option>${esc(m.name)}</option>`).join('')}</select></div><button class="mt-v316-btn primary" id="v316-query">조회</button><button class="mt-v316-btn" id="v316-print">인쇄</button><button class="mt-v316-btn green" id="v316-excel">엑셀</button></div></section><section class="mt-v316-paper" id="v316-result"><div class="mt-v316-empty">보고서를 선택하고 조회하세요.</div></section></main>`;
    document.querySelectorAll('[data-report]').forEach(b => b.addEventListener('click', () => renderReports(b.dataset.report)));
    document.getElementById('v316-query')?.addEventListener('click', queryReport);
    document.getElementById('v316-print')?.addEventListener('click', () => window.print());
    document.getElementById('v316-excel')?.addEventListener('click', exportReportExcel);
    document.getElementById('v316-manager-admin')?.addEventListener('click', openManagerAdmin);
    await queryReport();
  }
  ...
  function tableHtml(columns, rows) {
    ...
    return `<table class="mt-v316-table" id="v316-table"><thead><tr>${header}</tr></thead><tbody>${body || `<tr><td colspan="${columns.length}" class="mt-v316-empty">조회된 자료가 없습니다.</td></tr>`}</tbody></table>`;
  }
  ...
  async function queryReport() {
    ...
    const from = document.getElementById('v316-from')?.value || monthStart();
    const to = document.getElementById('v316-to')?.value || today();
    const manager = document.getElementById('v316-manager')?.value || '';
    const result = document.getElementById('v316-result');
    if (!result) return;
    result.innerHTML = '<div class="mt-v316-empty">조회 중...</div>';
    try {
      ...
      result.innerHTML = `<h2>${esc(cfg.title)}</h2><div class="period">${esc(from)} ~ ${esc(to)}${manager ? ' · '+esc(manager) : ''}</div>${tableHtml(columns, rows)}<div class="mt-v316-muted">머리글을 드래그하면 컬럼 순서를 바꿀 수 있습니다. 맨 아래 합계 행이 표시됩니다.</div>`;
      bindColumnDrag(result.querySelector('#v316-table'));
    } catch (e) { result.innerHTML = `<div class="mt-v316-empty" style="color:#b91c1c">${esc(e.message)}</div>`; }
  }
  ...
  function exportReportExcel() {
    const table = document.getElementById('v316-table');
  ...
  function stampUrl() { return new URL('/assets/mt_stamp.png?v=316', location.origin).href; }
  function statementCss() { return `@page{size:A4 portrait;margin:6mm}*{box-sizing:border-box}html,body{margin:0;background:#fff;color:#111;font-family:'Malgun Gothic',Arial,sans-serif}.toolbar{position:fixed;right:8mm;top:6mm;z-index:99999;display:flex;gap:8px}.toolbar button{border:0;border-radius:8px;padding:9px 15px;font-weight:900;cursor:pointer}.toolbar .print{background:#2563eb;color:#fff}.toolbar .close{background:#e5e7eb}.sheet{width:198mm;height:285mm;margin:0 auto;overflow:hidden}.copy{position:relative;height:136mm}.copy-label{position:absolute;right:0;top:0;font-size:11px}.cut{height:5mm;border-top:1px dashed #777;margin:1.2mm 0}.head,.items{width:100%;border-collapse:collapse;table-layout:fixed}.head td,.items td,.items th{border:1px solid #111;vertical-align:middle}.head td{height:7.2mm;padding:1mm 1.6mm;font-size:11px}.items td,.items th{height:6.8mm;padding:.8mm 1.3mm;font-size:11px}.title{font-size:22px!important;font-weight:900;text-align:center;letter-spacing:8mm;height:13mm!important}.no{font-weight:900}.side{writing-mode:vertical-rl;text-align:center;font-weight:800;letter-spacing:2px;background:#f3f4f6}.label{font-weight:800;text-align:center;background:#f8fafc}.center{text-align:center}.right{text-align:right}.claim{text-align:center;font-weight:800;height:6mm!important}.items th{background:#e5e7eb;text-align:center}.name{text-align:left;word-break:keep-all}.summary td{height:6.5mm;font-weight:800}.stamp{position:absolute;right:39mm;top:31mm;width:21mm;opacity:.9;z-index:4;transform:rotate(-8deg)}@media print{.toolbar{display:none}.sheet{width:198mm;height:285mm}.copy{page-break-inside:avoid}}`; }
  ...
  function patchStatementButtons(root=document){ root.querySelectorAll('.action-cell, td').forEach(cell=>{ if(cell.querySelector('[data-v316-statement]'))return; const src=cell.querySelector('[data-view], [data-edit-order], [data-ship], [data-delete-order]'); const id=src?.dataset.view||src?.dataset.editOrder||src?.dataset.ship||src?.dataset.deleteOrder; if(!id)return; const b=document.createElement('button'); b.type='button'; b.className='small secondary'; b.dataset.v316Statement=id; b.textContent='거래명세서'; cell.append(' ', b); }); }
  document.addEventListener('click', e=>{ const b=e.target.closest?.('[data-v316-statement], [data-order-action="statement"], [data-statement-print], [data-order-statement]'); if(!b)return; e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); openStatement(b.dataset.v316Statement||b.dataset.orderId||b.dataset.id||b.getAttribute('data-order-statement')); }, true);

  function runPatches() { injectStyle(); addReportsMenu(); hideUnnecessarySiteButtons(document); patchStatementButtons(document); document.querySelectorAll('input').forEach(formatNumberInput); document.querySelectorAll('form').forEach(ensureManagerField); }
