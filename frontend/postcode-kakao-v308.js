(function () {
  ...
  const LAYER_CLASS = 'mtoptics-postcode-v308-layer';
  ...
  function loadPostcodeScript() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.dataset.mtopticsPostcodeV308 = '1';
  ...
  function createLayer(title) {
    document.querySelectorAll('.' + LAYER_CLASS + ', .mtf-postcode-layer, .mtf-address-v304-layer, .mtf-address-v305-layer, .mtf-address-v306-layer, .mtf-address-v308-layer').forEach((n) => n.remove());
  ...
  function openPopup(scope, layer) {
    try {
      ...
      }).open({ popupKey: 'mtoptics-postcode-v308', popupTitle: 'MT옵틱스 주소검색', autoClose: true });
    } catch (err) {
  ...
  window.MTOpticsPostcodeV308 = { open, loadPostcodeScript };
  ...
  if (!window.__MTOPTICS_POSTCODE_V308_CAPTURE__) {
    window.__MTOPTICS_POSTCODE_V308_CAPTURE__ = true;
