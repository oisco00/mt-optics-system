(function () {
  ...
  const LAYER_CLASS = 'mtoptics-postcode-v307-layer';
  ...
  function loadPostcodeScript() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.dataset.mtopticsPostcodeV307 = '1';
  ...
  function createLayer(title) {
    document.querySelectorAll('.' + LAYER_CLASS + ', .mtf-postcode-layer, .mtf-address-v304-layer, .mtf-address-v305-layer, .mtf-address-v306-layer, .mtf-address-v307-layer').forEach((n) => n.remove());
  ...
  function openPopup(scope, layer) {
    try {
      ...
      }).open({ popupKey: 'mtoptics-postcode-v307', popupTitle: 'MT옵틱스 주소검색', autoClose: true });
    } catch (err) {
  ...
  window.MTOpticsPostcodeV307 = { open, loadPostcodeScript };
  ...
  if (!window.__MTOPTICS_POSTCODE_V307_CAPTURE__) {
    window.__MTOPTICS_POSTCODE_V307_CAPTURE__ = true;
