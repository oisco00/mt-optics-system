...
(() => {
  ...
  function repairMojibake(value) {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      if (!/[ÃÂâêëìíîïðñòóôõöøùúûüýþæçžœš]/i.test(current)) {
        break;
      }
      ...
      if (textScore(decoded) <= textScore(current)) break;
      current = decoded;
    }
  ...
  function injectStyles() {
    if (document.getElementById('mt-final-styles-v200')) return;

    const style = document.createElement('style');
    style.id = 'mt-final-styles-v200';
    style.textContent = `
      ...
      @media(max-width:700px){
        .mtf-filter-grid,.mtf-summary,.mtf-form-grid{grid-template-columns:1fr}
      ...
      function renderMatches() {
        ...
        list.innerHTML = matches.length
          ? matches.map((customer) => `
            ...
            `).join('')
          : `<div class="mtf-empty">일치하는 거래처가 없습니다.</div>`;
  ...
  async function renderCustomers(el, force = false) {
    ...
    el.innerHTML = `
      <div class="mtf-root" data-mtf-view="customers-v200">
        <div class="mtf-head">
          <div>
            <h1>거래처/원장</h1>
            <p>공란 검색은 전체 거래처를 가나다순으로 표시하며, 입력 즉시 포함 검색합니다.</p>
          </div>
          <div class="mtf-actions">
            ${can('customers.write')
  ...
  async function renderOrders(el) {
    ...
    el.innerHTML = `
      <div class="mtf-root" data-mtf-view="orders-v200">
        <div class="mtf-head">
          <div>
            <h1>주문/출고</h1>
            <p>기간·거래처·발송구분·상태를 각각 선택하고 미출고 주문과 즉시출고를 함께 처리합니다.</p>
          </div>
          <div class="mtf-actions">
            ${can('orders.write')
              ? `
    ...
    el.querySelector('[data-mtf-ac]')?.addEventListener(
    ...
    el.querySelector('[data-order-search]').addEventListener('click', () => {
  ...
  function paymentFilterQuery() {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(uiState.paymentFilters)) {
      if (value) params.set(key, value);
    }
    return params.toString();
  }
  ...
  async function renderPayments(el) {
    if (!el) return;
    const token = ++activeRenderToken;
    const [customers, sites, receivables, payments] = await Promise.all([
      loadCustomers(),
      loadSites(),
      api(`/final/receivables?${paymentFilterQuery()}`),
      api(`/final/payments?${paymentFilterQuery()}`)
    ]);
    if (token !== activeRenderToken || activePage() !== 'payments') return;

    uiState.selectedReceivables.clear();
    ...
    el.innerHTML = `
      <div class="mtf-root" data-mtf-view="payments-v200">
        <div class="mtf-head">
          <div>
            <h1>수금/미수금</h1>
            <p>거래처·납품처·발송구분 조건을 미수금과 최근 수금에 동시에 적용합니다.</p>
          </div>
          <div class="mtf-actions">
            ${can('payments.write')
              ? '<button class="mtf-btn warning" data-repair-korean>한글 깨짐 복구</button>'
              : ''}
          </div>
        </div>
        ...
        <div class="mtf-toolbar">
          <div class="mtf-filter-grid">
            ...
            <div class="mtf-field">
              ...
              <select class="mtf-select" id="mtf-payment-filter-site">
                ${siteOptions(sites, uiState.paymentFilters.customer_id, uiState.paymentFilters.site_id)}
              </select>
            </div>
            <div class="mtf-field">
              <label>발송구분</label>
          ...
          <div class="mtf-summary">
            <div class="mtf-stat"><span>조건 미수금</span><strong>${money(receivableTotal)}원</strong></div>
            <div class="mtf-stat"><span>미수 항목</span><strong>${money(receivables.length)}</strong></div>
        ...
        ${can('payments.write')
          ? paymentRegistrationHtml(customers, sites)
          : ''}

        <div class="mtf-card">
          <div class="mtf-head" style="align-items:center">
            <div>
              <h2 style="margin:0 0 4px">발송구분별 미수금</h2>
              <p>같은 거래처 내에서 여러 항목을 체크하면 합계가 자동 계산됩니다.</p>
            </div>
          </div>

          <div class="mtf-selected-bar" style="margin:12px 0">
            <div>
              선택 <strong id="mtf-selected-count">0건</strong>
              · 합계 <span class="mtf-selected-total" id="mtf-selected-total">0원</span>
            </div>
            <button class="mtf-btn success" data-apply-selected disabled>
              선택합계 수금등록에 반영
            </button>
          </div>
          ...
          <div class="mtf-table-wrap">
            <table class="mtf-table">
              <thead>
                <tr>
                  <th>선택</th><th>거래처</th><th>납품처</th><th>발송구분</th>
                  <th>매출</th><th>수금</th><th>미수금</th>
                </tr>
              </thead>
              <tbody>
                ${receivables.length
                  ? receivables.map((row, index) => `
                    ...
                    `).join('')
                  : '<tr><td colspan="7" class="mtf-empty">조건에 해당하는 미수금이 없습니다.</td></tr>'}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div class="mtf-card">
          ...
          <p style="color:#64748b">기본은 최근 1개월이며, 최근 날짜 역순으로 표시합니다.</p>
          <div class="mtf-table-wrap">
            <table class="mtf-table">
              <thead>
                <tr>
                ...
                ${payments.length
                  ? payments.map((payment) => `
                      <tr>
                        ...
                        <td>${escapeHtml(repairMojibake(payment.approval_no || payment.memo || ''))}</td>
                        <td>
                          ${can('payments.write')
                            ? `
                              <div class="mtf-actions">
    ...
    filterWrapper?.addEventListener('mtf-customer-selected', (event) => {
      uiState.paymentFilters.customer_id = event.detail?.id || '';
      uiState.paymentFilters.customer_q = event.detail?.name || '';
      filterSite.innerHTML = siteOptions(
        sites,
        event.detail?.id || '',
        ''
      );
    ...
    el.querySelector('[data-payment-search]').addEventListener('click', () => {
      uiState.paymentFilters.customer_q =
        el.querySelector('#mtf-payment-filter-customer').value.trim();
      uiState.paymentFilters.site_id = filterSite.value;
    ...
    bindPaymentRegistration(el, customers, sites);

    el.querySelectorAll('[data-receivable-index]').forEach((checkbox) => {
      checkbox.addEventListener('change', () => {
        const row = receivables[Number(checkbox.dataset.receivableIndex)];
        if (!checkbox.checked) {
          uiState.selectedReceivables.delete(
            `${row.customer_id}:${row.customer_site_id || 0}:${row.delivery_type}`
          );
          updateSelectedSummary(el);
          return;
        }

        const selected = [...uiState.selectedReceivables.values()];
        if (
          selected.length &&
          String(selected[0].customer_id) !== String(row.customer_id)
        ) {
          checkbox.checked = false;
          showToast(
            '한 번의 수금등록에는 같은 거래처의 미수금만 선택할 수 있습니다.',
            'error'
          );
          return;
        }

        uiState.selectedReceivables.set(
          `${row.customer_id}:${row.customer_site_id || 0}:${row.delivery_type}`,
          row
        );
        updateSelectedSummary(el);
    ...
    el.querySelector('[data-apply-selected]').addEventListener('click', () => {
      applySelectedReceivables(el, customers, sites);
    });

    el.querySelector('[data-repair-korean]')?.addEventListener(
      'click',
      async () => {
        const ok = await confirmModal(
          '한글 깨짐 복구',
          '기존 수금 비고와 엑셀 파일명의 복구 가능한 깨진 한글을 일괄 보정합니다.',
          '복구 실행'
        );
        if (!ok) return;
        ...
        showToast(`${result.updated}건의 한글을 복구했습니다.`);
        renderPayments(el);
      }
    );
    ...
    el.querySelectorAll('[data-payment-edit]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        openPaymentEditModal(payment, customers, sites);
      });
    });
  ...
  function paymentRegistrationHtml(customers, sites) {
    return `
      <div class="mtf-card" id="mtf-payment-register-card">
        <h2 style="margin-top:0">수금 등록</h2>
          ...
          <div class="mtf-form-grid">
            ...
            <div class="mtf-field">
              ...
              <input class="mtf-input" type="number" min="1" name="amount"
                     id="mtf-payment-amount" required>
            </div>
            <div class="mtf-field">
              <label>카드사/은행</label>
  ...
  function bindPaymentRegistration(el, customers, sites) {
    ...
    wrapper.addEventListener('mtf-customer-selected', (event) => {
      const customer = event.detail;
      siteSelect.innerHTML = siteOptions(sites, customer?.id || '');
      if (!customer) return;
  ...
  function updateSelectedSummary(el) {
    const rows = [...uiState.selectedReceivables.values()];
    const total = rows.reduce(
      (sum, row) => sum + Number(row.receivable_balance || 0),
      0
    );

    el.querySelector('#mtf-selected-count').textContent =
      `${rows.length}건`;
    el.querySelector('#mtf-selected-total').textContent =
      `${money(total)}원`;
    el.querySelector('[data-apply-selected]').disabled = rows.length === 0;
  }

  function applySelectedReceivables(el, customers, sites) {
    const rows = [...uiState.selectedReceivables.values()];
    ...
    const customer = customers.find(
      (row) => String(row.id) === String(first.customer_id)
    );
    const sameSite = rows.every(
      (row) =>
        String(row.customer_site_id || '') ===
        String(first.customer_site_id || '')
    );
    const sameDelivery = rows.every(
      (row) => String(row.delivery_type) === String(first.delivery_type)
    );

    const form = el.querySelector('#mtf-payment-form');
    const acInput = form.querySelector('#mtf-payment-form-customer');
    const acHidden = form.querySelector('[name="customer_id"]');
    ...
    const amount = form.querySelector('#mtf-payment-amount');
    const memo = form.querySelector('#mtf-payment-memo');

    acInput.value = customer?.name || first.customer_name;
    acHidden.value = first.customer_id;
    siteSelect.innerHTML = siteOptions(
      sites,
      first.customer_id,
      sameSite ? first.customer_site_id : ''
    );
    if (sameSite) siteSelect.value = first.customer_site_id || '';
    deliverySelect.value = sameDelivery ? first.delivery_type : '기타';
    amount.value = Math.max(total, 0);
    memo.value = [
      '선택 미수금 합계 수금',
      ...rows.map((row) =>
        `${row.site_name || '기본'} / ${row.delivery_type} / ${money(row.receivable_balance)}원`
      )
    ].join('\n');
    ...
    amount.focus();
    showToast('선택한 미수금 합계를 수금등록에 반영했습니다.');
  }

  function openPaymentEditModal(payment, customers, sites) {
    const modal = openModal(
        ...
        <div class="mtf-form-grid">
          ...
          <div class="mtf-field">
            ...
            <select class="mtf-select" name="customer_site_id"
              ...
              ${siteOptions(sites, payment.customer_id, payment.customer_site_id || '')}
            </select>
          </div>
          <div class="mtf-field">
            <label>발송구분</label>
    ...
    wrapper.addEventListener('mtf-customer-selected', (event) => {
      siteSelect.innerHTML = siteOptions(sites, event.detail?.id || '');
      const first = sites.find(
        (site) => String(site.customer_id) === String(event.detail?.id || '')
      );
      if (first) {
        siteSelect.value = first.id;
        deliverySelect.value = first.default_delivery_type || '택배';
  ...
  async function renderCurrentPage(force = false) {
    ...
    const marker = el.querySelector(`[data-mtf-view="${page}-v200"]`);
